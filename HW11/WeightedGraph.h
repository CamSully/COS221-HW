...
vector<WeightedEdge>& edges)
{
	...
	addVertex(i); // vefor (unsigned i = 0; i < edges.size(); i++)
	{
	addEdge(edges[i].u, edges[i].v, edges[i].weight);
	}
}
...
{
	vector<int> T; // T contains the vertices in the tree
	vector<int> parent(getSize()); // Parent of a verte
...
template<typename V>
ShortestPathTree WeightedGraph<V>::getShortestPath(int
sourceVertex)
{
